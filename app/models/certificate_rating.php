<?php
/**
* CertificateRating Model class
*/
class CertificateRating extends AppModel {
	var $name = 'CertificateRating';
	...
	function get_avg_rating(){
		...
		$full_stars = 0;
		$avg_rating_temp = 0;
		$all_ratings = $this->find('all');
		
		if(!empty($all_ratings)){
			foreach($all_ratings as $rating){
				$total_rating = $total_rating+$rating['CertificateRating']['rating'];
			}
