<?php
/**
* FeedbackRating Model class
*/
class FeedbackRating extends AppModel {
	var $name = 'FeedbackRating';
	var $assocs = array(
			'Feedback' => array(
				'type' => 'belongsTo',
				'className' => 'Feedback',
	...
	function get_avg_rating($product_id = null){
		...
		$full_stars = 0;
		$avg_rating_temp = 0;
		$all_ratings = $this->find('all',array('conditions'=>array('FeedbackRating.order_item_id'=>$product_id)));
		
		if(!empty($all_ratings)){
			foreach($all_ratings as $rating){
				$total_rating = $total_rating+$rating['FeedbackRating']['rating'];
			}
