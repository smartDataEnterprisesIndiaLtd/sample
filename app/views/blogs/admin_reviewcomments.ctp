...
?>
	...
	<tr>
		...
		<?php if( count($blogcomments) > 1 ) echo '('.count($blogcomments).' Comments)'; else  echo '('.count($blogcomments).' Comment)';?> </td>
		<td class="adminGridHeading" height="25px" align="right">
			<?php
			
			echo $html->link('Add New Comment','/admin/blogs/addcomment/'.$blog_id)?>&nbsp;&nbsp;
		</td>
	</tr>
				...
				<tr>
					<td>
						<table width="100%" cellspacing="1" cellpadding="2" class="adminBox" align="center" border="0">
							<tr>
								<td>
									<?php echo $form->create("Blog",array("action"=>"reviewcomments/".$blog_id,"method"=>"Post", "id"=>"frmSearchBlog", "name"=>"Blog"));
									echo $form->hidden('BlogComment.blog_id',array('label'=>false,'div'=>false,'value'=>$blog_id));
									?>
	...
	<tr>
		<td colspan="2" id="blog_pagging">
			<?php echo $this->element('blogs/blog_comment_listing'); ?>
		</td>
	</tr>
	<tr><td>&nbsp;</td></tr>
	<tr>
