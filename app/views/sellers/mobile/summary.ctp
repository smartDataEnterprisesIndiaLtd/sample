...
<!--Star Rating Starts-->
	...
	<li class="starsfdbk">
		...
		<?php //echo '<span style="color:#0033AC">Browse</span> '.$html->link($seller_info['Seller']['business_display_name'].'\'s  ','/sellers/store/'.$seller_id,array('escape'=>false,'class'=>'underline-link'));?>
	...
	<h4>Recent Feedback
		<?php echo $html->link('See all feedback','/sellers/feedback/'.$seller_id.'/'.$product_id.'/'.$condition_id,array('escape'=>false,'class'=>'font11'));?>
	</h4>
	...
	<!---->
		<ul class="fdbkbyr">
			...
			<li class="fbbkbyrsstr">
				<?php $common->displaySellerrating($recent_feedback['Feedback']['rating']);?>
			</li>
			<li>"
				<?php if(!empty($recent_feedback['Feedback']['feedback'])) echo $recent_feedback['Feedback']['feedback']; ?>
			"</li>
			<li class="font11 gray">Buyer: 
				<?php echo $recent_feedback['User']['firstname'];?>&nbsp;| 
				<?php echo date(DATE_FORMAT,strtotime($recent_feedback['Feedback']['created']));?>
			</li>
		</ul>
	<!---->
	<?php }?>
		...
		<ul>
				...
				if(!empty($pro_seller_info['UserSummary']['SellerSummary'])){
					...
					<li class="margin-top"><span class="frydlvry drkred">FREE DELIVERY</span>&nbsp;<b class="font11">Spend over 
							...
							with this seller for free standard UK Delivery</b>
					</li>
				<?php } } ?>
