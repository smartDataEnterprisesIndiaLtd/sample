...
* @return string html for the list of page numbers
**/
    function albumNumbers($separator=null,$escapeTitle=true,$spacerLower="...",$spacerUpper="...")
    {
        if (empty($this->_pageDetails) || $this->_pageDetails['pageCount'] == 1) { return "<div class='pagination-bg1'><em>1</em></div>"; }
        $total = $this->_pageDetails['pageCount'];
        $max = $this->maxPages;
        $span = $this->pageSpan;
        if ($total<$max)
        ...
        {
            $lowerLimit = $lowerLimit+1;
            $t[] = "<div class='pagination-bg1'>".$this->_generateLink(1,1,$escapeTitle)."</div>";
            if ($spacerLower)
            {
                $t[] = "<div class='pagination-bg1'>".$spacerLower."</div>";
            }
        }
        if (($upperLimit<>$total)AND($this->showLimits))
        {
        ...
        for ($i = $lowerLimit; $i <= $upperLimit; $i++)
        {
             ...
             {
                $text = '<em>'.$i.'</em>';
			$text = "<div class='pagination-bg'>".$text."</div>";
             }
             else
             {
                $text = $this->_generateLink($i,$i,$escapeTitle);
			$text = "<div class='pagination-bg1'>".$text."</div>";
             }
             $t[] = $text;
        }
        if ($dottedUpperLimit)
        {
            if ($spacerUpper)
            {
                $t[] = "<div class='pagination-bg1'>".$spacerUpper."</div>";
            }
            $t[] = "<div class='pagination-bg1'>".$this->_generateLink($this->_pageDetails['pageCount'], $this->_pageDetails['pageCount'],$escapeTitle)."</div>";
        }
        $t = implode($separator, $t);
	 ...
	 function photoNumbers($separator=null,$escapeTitle=true,$spacerLower="...",$spacerUpper="...")
    { 
        if (empty($this->_pageDetails) || $this->_pageDetails['pageCount'] == 1) { return "<div class='pagination-bg2'><em>1 / 1</em></div>"; }
        $total = $this->_pageDetails['pageCount'];
        $max = $this->maxPages;
        $span = $this->pageSpan;
        if ($total<$max)
        ...
        for ($i = $lowerLimit; $i <= $upperLimit; $i++)
        {
             ...
             {
                $text = '<em>'.$i.'</em>';
				$text = "<div class='pagination-bg2'>".$text.' / '.$this->_pageDetails['pageCount']."</div>";
             }
             else
             {
                //$text = $this->_generateLink($i,$i,$escapeTitle);
...
**/
    function _generateLink ($title,$page=NULL,$escapeTitle,$pageDetails = NULL) 
    {
        ...
        if ($this->style=="ajax")
        {
            ...
            if (!empty($pageDetails['ajaxFormId']))
            {
                $id = 'link' . intval(rand());
