...
if((err_msg_alert != '') && (jQuery("#fancybox-content",parent.document).height() == 285))
{
...
<?php  echo $form->create('',array('action'=>'email_friend','method'=>'POST','name'=>'frmProductmail','id' =>'frmProductmail'));
...
<ul class="pop-con-list" style="margin-right:10px;">
	...
	<?php
            if(isset($success) && ($success ==1)){
                echo "<script type=\"text/javascript\">setTimeout('parent.jQuery.fancybox.close()',5000);
                                            </script>";
            }
        }
	?>
	...
	<li>
		...
		<p class="gr-colr">Complete the form below to email details of this page to a friend.</p>
	</li>
		...
		<ul class="ad-form-widget" style="width:385px !important;">
			<li>
				<?php echo $form->hidden('Product.id'); ?>
                                <?php echo $form->hidden('Product.url',array('id'=>'URLID','value'=>''));?>
			</li>
			<li>
...
?>
<script>    
    document.getElementById('URLID').value = parent.window.location.pathname;
</script>