<?php 
if(empty($main_image['Product']['minimum_price_value']) ){
		$main_image['Product']['minimum_price_value'] = '0';
	}
	if(empty($main_image['Product']['minimum_price_seller']) ){
		$main_image['Product']['minimum_price_seller'] = '0';
	}
	if(empty($main_image['Product']['minimum_price_used']) ){
		$main_image['Product']['minimum_price_used'] = '0';
	}
	if(empty($main_image['Product']['minimum_price_used_seller']) ){
		$main_image['Product']['minimum_price_used_seller'] = '0';
	}
	$newLowestPriceSeller  = trim($main_image['Product']['minimum_price_seller']);
	$usedLowestPriceSeller = trim($main_image['Product']['minimum_price_used_seller']);
	$rrp_price 	       = trim($main_image['Product']['product_rrp']);
	$product_id	       = trim($main_image['Product']['id']);
	
	if(!empty($newLowestPriceSeller) ){  // IF seller exists for new condition 
			
		$lowest_price           = $main_image['Product']['minimum_price_value'];
		$lowest_price_seller_id = $main_image['Product']['minimum_price_seller'];
		$product_condition 	= $main_image['Product']['new_condition_id'];
	 ...
	 ######################## USED PRODUCT SELLER #####################################
		...
		$lowest_price           = $main_image['Product']['minimum_price_used'];
		$lowest_price_seller_id = $main_image['Product']['minimum_price_used_seller'];
		$product_condition 	= $main_image['Product']['used_condition_id'];
	...
	$this->set('product_id',$main_image['Product']['id']);
?>
...
<section class="maincont nopadd">
			...
			<h2><?php echo $main_image['Product']['product_name'];?></h2>
			...
			<ul>
					...
					<section class="prdctboxmain">
						...
						<?php
							if(!empty($main_image['Product']['image'])){
								$main_imagePath2 = WWW_ROOT.PATH_PRODUCT.'large/img_300_'.$main_image['Product']['image'];
								if(file_exists($main_imagePath2)){
									echo $html->image('/'.PATH_PRODUCT.'large/img_300_'.$main_image['Product']['image'], array('alt'=>""));
								}else{
				...
				<?php
				if(!empty($main_image['Productimage'])) {
					foreach($main_image['Productimage'] as $pro_image){
				...
				<li class="margin-top" style="text-align:center"><b><?php if($stockInfo['ProductSeller']['quantity'] != 0 ||  !empty($stockInfo['ProductSeller']['quantity'])  ) {?> Quantity: <?php }?></b>
				 <?php
					if( empty($stockInfo) ) {
						...
						<?php $addToBasket =  $main_image['Product']['id'].",'prod_quantity_id',";
