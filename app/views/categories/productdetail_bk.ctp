...
<div class="mid-content pro-mid-content">
	...
	<div class="product-preview-widget">
			...
			<div style="height:200px; width:200px;text-align:center; vertical-align:middle;">
				...
				if(!empty($product_details['Product']['product_image'])){
					...
					if(file_exists($main_imagePath)){
						...
						echo $html->image('/img/no_image_200.jpg', array('alt'=>$product_details["Product"]["product_name"],'title'=>$product_details["Product"]["product_name"]));
					}
		...
		<div class="product-details-sec">
			<h1><?php if(!empty($product_details['Product']['product_name'])) echo $product_details['Product']['product_name'];?></h1>
			<?php
			######################## NEW PRODUCT SELLER #####################################
			if(!empty($newLowestPriceSeller) ){  // IF seller exists for new condition 
				$lowest_price = $product_details['Product']['minimum_price_value'];
			...
			<ul class="pro-details">
				...
				<li>
				<?php
				if( !empty($lowest_price_seller_id) ){ ?> <!--// if sellers exists for this products-->
					...
					<?php }else if($stockInfo['ProductSeller']['quantity'] <= 0 ||  empty($stockInfo['ProductSeller']['quantity'])  ){ ?>
						<p class="price lrgr-fnt"><strong>Sorry - temporarily out of stock</strong></p>
					<?php }else{ ?>
						<p class="green-color larger-fnt margin-top"><strong>In Stock</strong></p>
					<?php } ?>
	...
	<div class="row no-pad-btm">
		...
		<div class="fbtogether">
			...
			<!--Product Description Start-->
			<div class="product-des content-list">
				<?php
				if(!empty($product_details['ProductDetail']['description'])){ 
					echo html_entity_decode($product_details['ProductDetail']['description']); 
					//echo htmlspecialchars($product_details['ProductDetail']['description']);
				} ?>
				<?php
	...
	<div class="smalr-fnt margin-top" style="border-top: 20px solid #EEEEEE;overflow: hidden;margin-bottom:10px;"></div>
	<!--Customers Viewing This Page May Be Interested in These Sponsored Links Start-->
