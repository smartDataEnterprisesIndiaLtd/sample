...
<section class="forall fbtogetther-items">
		<?php
		...
		foreach($prods_fre_togather as $freq_prod){
			...
			?>
		<p class="thumb-imgs allitems hide_<?php echo $pr_id;?>" style="padding-right: 0;">
				<?php if($i == 1){
						echo $html->image($cus_image_path,array('alt'=>$freq_prod['product_name'] ,'title'=>$freq_prod['product_name'],'width'=>'30','height'=>'30' ));
					}else{?>
						<?php echo $html->image('mobile/plus_icon.png', array('alt'=>'','width'=>'12','height'=>'12','class'=>'plusicon tog_'.$i));?>
						<?php echo $html->link($html->image($cus_image_path,array('alt'=>$freq_prod['product_name'] ,'title'=>$freq_prod['product_name'],'width'=>'30','height'=>'30' )),'/'.$this->Common->getProductUrl($pr_id).'/categories/productdetail/'.$pr_id,array('escape'=>false));
		...
		<!---->
		<section class="allprice" style="padding-left: 14px;">
		<?php
				$mix_prods_str = '';
				foreach($prods_fre_togather as $freq_prod){
					//pr($freq_prod);
				...
				?>
		Price for all <?php echo CURRENCY_SYMBOL;?><span id = "total_freq_price"><?php echo $format->money($total_price_added,2)?></span>
		...
		<p>Some of these items may be dispatched sooner</p>
		<p class="sellmor">List and Sell for Free on Choiceful.com
		<?php $pUrl = SITE_URL."marketplaces/view/choiceful-marketplace-sellers-guide"; ?>
		<a href="<?php echo $pUrl; ?>">(find out more)</a></p>
		<p>
			<a class="bluegradbtn" href="<?php echo SITE_URL ?>marketplaces/create_listing/<?php echo $product_details['Product']['id'] ?>">
			Sell yours here
			</a>
		
		<!--- <input type="button" value="Sell yours here" class="bluegradbtn" /> --></p>
</section>
<!---->
<?php } }?>
