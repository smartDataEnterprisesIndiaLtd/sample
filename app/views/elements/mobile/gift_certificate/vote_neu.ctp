<?php 
$savedGiftVote_neu = $this->Session->read('giftVotesaved_neu'.$rev_gift_neu_id);
...
if(!empty($rev_gift_neu_id)){
	$like_gift_votes = $this->CertificateReviewVote->find('count',array('conditions'=>array('CertificateReviewVote.review_id'=>$rev_gift_neu_id,'CertificateReviewVote.user_vote'=>'1')));

	$dislike_gift_votes = $this->CertificateReviewVote->find('count',array('conditions'=>array('CertificateReviewVote.review_id'=>$rev_gift_neu_id,'CertificateReviewVote.user_vote'=>'0')));
}
...
<?php 
if(empty($savedGiftVote_neu)){
?>
<li>
	<div class="like-div-padding">
	<?php 
			echo $html->link($html->image("like-this-icon.png",array('width'=>"58",'height'=>"25", 'alt'=>"" ,'class'=>'img pad-rt')),'javascript::void(0)', array('Onclick'=>'likereview('.$rev_gift_neu_id.',1,1)','escape'=>false), null,false);
		?>
		<?php 
			echo $html->link($html->image("dislike-this.png",array('width'=>"32",'height'=>"25", 'alt'=>"" ,'class'=>'img pad-rt')),'javascript::void(0)', array('Onclick'=>'likereview('.$rev_gift_neu_id.',0,1)','escape'=>false), null,false);
		?>
	</div>
...
function likereview(rev_gift_pos_id, att1, att2){
	...
	jQuery.ajax({
		...
		/** Update the div**/
		jQuery('#vote_neu_'+rev_gift_pos_id).html(msg);
