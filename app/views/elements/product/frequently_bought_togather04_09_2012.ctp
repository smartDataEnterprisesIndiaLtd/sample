			...
			<div class="items-widget">
				...
				foreach($prods_fre_togather as $freq_prod){
					...
					if($i == 1){
						...
						echo $html->link($html->image($cus_image_path,array('alt'=>$freq_prod['product_name'] ,'title'=>$freq_prod['product_name'] )),'/'.$this->Common->getProductUrl($pr_id).'/categories/productdetail/'.$pr_id,array('escape'=>false));
					
				
					}
					 if($i < count($prods_fre_togather)) {?> <span class="plus-span">+</span><?php }
				$i++;
				}
				?>
			</div>
			...
			if(!empty($prods_fre_togather_seller)){
				...
				<p>
					...
					if(!empty($freq_prod_seller['sale_price']) && ($freq_prod_seller['qty'] > 0)){
						...
						echo $form->checkbox('select_'.$freq_prod_seller['id'],array('value'=>$freq_prod_seller['sale_price'],'id'=>$freq_prod_seller['id'],'checked'=>'checked','onClick'=>'changeValuesForBasket(this.id,'.$pri.','.$s_id.','.$c_id.');','style'=>array('border:0')));
					} ?> 
					<?php
					if($j == 1){ ?>
