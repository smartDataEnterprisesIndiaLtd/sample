...
<!--Sorting Start-->
		...
		if(!empty($prev_page)){
                        ...
                        $link_url =  $this->params['controller'].'/'.$this->params['action'].'/'.$selected_category.'/'.$department_id.'/'.'&'.$url_fh_paging.'&fh_start_index='.($prev_page[0]-1)*$view_size.'&fh_view_size='.$view_size.'&';
                        echo "<link rel='prev' href='".SITE_URL.$link_url."'>";
                           
                    }
                    if(!empty($next_page)){ 
                            
                        $link_url_next = $this->params['controller'].'/'.$this->params['action'].'/'.$selected_category.'/'.$department_id.'/'.'&'.$url_fh_paging.'&fh_start_index='.($next_page[0]-1)*$view_size.'&fh_view_size='.$view_size.'&';
