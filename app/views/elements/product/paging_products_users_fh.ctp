...
<div class="paging">
	<ul>
		...
		<?php if(!empty($prev_page)){ ?>
			...
			<li><?php echo $html->link('Prev',$this->params['action'].'/'.$searchWord.'/&'.$url_fh_paging.'&fh_start_index='.((($results['current_set']-1) * $view_size) - $view_size ).'&fh_view_size='.$view_size.'&',array('escape'=>false,'class'=>'active')); ?></li>
			<?php if(!empty($not_shown_pages_prev) && $not_shown_pages_prev > 0) { ?>
				<!--li>...</li-->
			<?php }?>
			<?php foreach($prev_page as $pre_p){ ?>
				<li><?php echo $html->link($pre_p,$this->params['action'].'/'.$searchWord.'/&'.$url_fh_paging.'&fh_start_index='.($pre_p-1)*$view_size.'&fh_view_size='.$view_size.'&',array('escape'=>false,'class'=>'active')); ?></li>
			<?php }
		}?>
		...
		<?php if(!empty($next_page)){ ?>
			<?php foreach($next_page as $next_p){ ?>
				<li><?php echo $html->link($next_p,$this->params['action'].'/'.$searchWord.'/&'.$url_fh_paging.'&fh_start_index='.($next_p-1)*$view_size.'&fh_view_size='.$view_size.'&',array('escape'=>false,'class'=>'active')); ?></li>
			<?php } ?>
			<li><?php echo $html->link('Next',$this->params['action'].'/'.$searchWord.'/&'.$url_fh_paging.'&fh_start_index='.($results['current_set'] * $view_size).'&fh_view_size='.$view_size.'&',array('escape'=>false,'class'=>'active')); ?></li>
