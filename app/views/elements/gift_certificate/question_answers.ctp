	...
	<div class="fbtogether">
		...
		<div class="tec-details">
			...
			if(!empty($logg_user_id))
					$ask_question_link = '/certificates/add_question/';
				else
					$ask_question_link = '/users/sign_in/';
			?>
			<?php echo $html->link('Ask a question',$ask_question_link,array('escape'=>false,'class'=>'underline-link ansque'));?></p>
			<?php if(!empty($questions)) { ?>
			<ul class="faq-sec">
				<?php foreach($questions as $que){?>
				<li>
					<p><span class="red-color pad-rt larger-font"><strong>Q</strong></span> <?php echo $que['CertificateQuestion']['question'];?>
					<?php
					if(!empty($logg_user_id))
					...
					<p><span class="red-color pad-rt larger-font"><strong>A</strong></span> <?php echo $answer['answer']; ?> <?php echo $html->image("smiley.png",array('width'=>"10",'height'=>"9", 'alt'=>"" ,'class'=>'img')); ?></p>
					<p id = <?php echo "voteans_".$answer['id'];?>>
					<?php echo $this->element('gift_certificate/vote_ans');?></p>
					<?php }?>
					<?php }?>
				</li>
				<?php }?>
