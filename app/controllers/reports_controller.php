...
class ReportsController extends AppController{
	...
	*/	
	...
	function admin_marketplace_highest_products(){
		...
		App::import('Model', 'VReport');
		$this->VReport = new VReport();
		...
		$this->paginate = array(
			'limit' => $limit,
			//'conditions'=>array(''),
			'fields' => array('VReport.user_id','VReport.business_display_name','VReport.firstname','VReport.lastname','VReport.totalproduct','VReport.totalrating')
			);
		$allSellerProducts = $this->paginate('VReport');
		$this->set('allSellerProducts' , $allSellerProducts);
		
		foreach($allSellerProducts as $allSellerProduct){
			$totalRating[$allSellerProduct['VReport']['user_id']] = $this->Common->positivePercentFeedback($allSellerProduct['VReport']['user_id']);
		}
		$this->set('totalRating' , $totalRating);
		
		$refundRate = $this->admin_refund_rates();
		$this->set('refundRate' , $refundRate);
		$shippedRates = $this->admin_late_shipped_rates();
		$this->set('shippedRates' , $shippedRates);
		$cancelRates = $this->admin_preshipped_cancel_rates();
		$this->set('cancelRates',$cancelRates);
		
		$orderSeller = array();
		
		$orderSeller = $this->admin_sellerOrder();
		$this->set('orderSeller',$orderSeller);
		
		$orderSellerFeedback = array();
		
		$orderSellerFeedback = $this->admin_feedback();
		$this->set('orderSellerFeedback',$orderSellerFeedback);
	}
	
	
	function admin_sellerOrder(){
		$this->loadModel('OrderView');
		$orderSeller =$this->OrderView->find('all',array('conditions' => array(" OrderView.payment_status = 'Y' AND OrderView.deleted != '1' "),'fields'=>array('OrderView.order_id','OrderView.seller_id'),'group' =>array('order_id','seller_id')));
		//$orderSeller = $this->OrderSeller->find('all',array('conditions'=>array('OrderSeller.seller_id != '=>253708*/),'fields'=>array('order_id','seller_id'),'group'=>array('order_id','seller_id')));
		//pr($orderSeller);
		$totalSellerOrder= array();
		
		foreach($orderSeller as $record=>$order){
			$order_id ='';
			$seller_id= '';
			
			$order_id = trim($order['OrderView']['order_id']);
			$seller_id = trim($order['OrderView']['seller_id']);
			$seller_id = trim($seller_id);
			$totalSellerOrder[$seller_id][] = $order_id;	
		}
		
			
		return $totalSellerOrder;
	}
	function admin_feedback(){
		$this->loadModel('Feedback');
		$orderSellerFeedback = $this->Feedback->find('all',array('conditions'=>array(/*'OrderSeller.seller_id'=>253708*/),'fields'=>array('order_id','seller_id')/*,'group'=>array('order_id','seller_id')*/));
		//pr($orderSeller);
		$totalSellerOrderFeedback= array();
		foreach($orderSellerFeedback as $record=>$order){
			$order_idFeedback ='';
			$seller_idFeedback= '';
			
			$order_idFeedback = trim($order['Feedback']['order_id']);
			$seller_idFeedback = trim($order['Feedback']['seller_id']);
			$seller_idFeedback = trim($seller_idFeedback);
			$totalSellerOrderFeedback[$seller_idFeedback][] = $order_idFeedback;	
		}
		return $totalSellerOrderFeedback;
	}
	/**
	...
	*/		
	...
	function admin_getDateWiseFinancialReport( $month_year_data ){
		...
		for($d = 1; $d <= $monthDays; $d++){
			...
			//Change the payment getway
			//$ccPayments = $this->getSaleByPaymentMethod( $sDateTime, $eDateTime , 'Sage' ); // Sage
			$ccPayments = $this->getSaleByPaymentMethod( $sDateTime, $eDateTime , 'Credit/Debit Card' ); // PayPal Express check out
	...
	*/
	function getPreShippedCancel($sDateTime, $eDateTime ){
		...
		$dayOrdersList = $this->Order->find('list',array('conditions'=>array(
			'Order.created >= "'.$sDateTime.'" AND Order.created <= "'.$eDateTime.'"',
			'Order.payment_status = "Y" AND Order.deleted !=  "1" ')
		));
		//pr($dayOrdersList);
		if(is_array($dayOrdersList) && count($dayOrdersList) > 0){
			...
			//echo $dayOrderStr;
			$refundData = $this->OrderRefund->find('all',array('conditions'=>array('OrderRefund.order_id IN('.$dayOrderStr.')'),
					'fields'=> array( 'SUM(amount) as refund_amount ' )
					));
			$refundData = $refundData[0][0]['refund_amount'];
			if(!empty($refundData)){
	...
	function admin_getSellerWiseFinancialReport( $day_month_year_data ){
		...
		if( count($sellerDataArr) > 0 ){
			foreach($sellerDataArr as $id=>$sData){
				...
				$sellerWiseDataArr[$id]['refund_amount'] = $refundedAmount;
				//$new_revenue = 	 $sData[0]['revenue']-$totalCancelledAmount;
				$new_revenue = 	 $sData[0]['revenue']-$refundedAmount;
	...
	function admin_preshipped_cancel_rates(){
		...
		$this->paginate = array(
			...
			foreach($cancelItemAmount as $key => $cancelItemAmount){
				...
				$arrCancleSeller[$cancelItemAmount['CanceledItem']['seller_id']]=round(($cc/$ts*100),2);
			}
		$this->set('arrCancle' , $arrCancle);
		return $arrCancleSeller;
	...
	function admin_late_shipped_rates(){
		...
		$lateShipmentRates = $this->OrderSeller->find('all',array('conditions'=>array(
			...
			if(empty($number_of_orders_history))
				...
				$lateShipmentRatesArrSeller[$lateShipmentRatesArr[$key]['seller_id']] = round($lateship_rate, 2);
				
			}
			
		$this->set('lateShipmentRatesArr' , $lateShipmentRatesArr);
		
		//Fro Display in admin_marketplace_highest_products function
		return $lateShipmentRatesArrSeller;
	}
	...
	function admin_refund_rates(){
		...
		if(!empty($refundedOrder)){
			if(empty($total_amount))
				...
				//Added for get a value with respect to seller
				$arrRefundedOrdesSeller[$arrRefundedOrder[$key]['seller_id']] = round((($refundedOrder[0]['total_refund'] / $total_amount) * 100), 2);
			}
		}	
		$this->set('arrRefundedOrder' , $arrRefundedOrder);
		return $arrRefundedOrdesSeller;
...
/** 
	...
	*/		
	...
	// Sending Data to google shopping..
	/*function googlecontent() {
		Configure::write('debug', 2);
		App::import('Vendor', 'PhpUploader', array('file' => 'googleshopping'.DS.'GShoppingContent.php'));
			...
			//$product->setDescription($product_list['PeeriusAttribute']['description']);
			$link = $product_list['0']['product_URL'];
			$product->setProductLink($link);
			$product->setSKU($product_list['PeeriusAttribute']['quick_code']);
			$product->setTargetCountry('UK');
			$product->setContentLanguage('en');
			$product->setBrand($product_list['PeeriusAttribute']['brand']);
			$product->setCondition('new');
			$product->setGtin($product_list['PeeriusAttribute']['manufacturer']);
			$product->setQuantity('200');
			$product->setAvailability('in stock');	
			$product->setPrice($product_list['0']['price'], 'gbp');
			//$product->setChannel('y');
