			...
			if(!empty($userValidate)){
				...
				if($this->User->save()){
					...
					*/
					...
					$this->set('data',$data);
					$this->Email->to = ucfirst($this->data['User']['firstname']).' '.ucfirst($this->data['User']['lastname']).'<'.$this->data['User']['email'].'>';
					/******import emailTemplate Model and get template****/
...
// 						}
			...
			if(empty($sessOrderData) || count($sessOrderData) <= 0){
				...
				case 'PayPal': // Paypal
					...
					if ($strConnectTo == "LIVE"){
						...
						$this->Paypal->add_field('business', 'ariansuk@googlemail.com');
					} else{
						...
						$this->Paypal->add_field('business', 'kul_1303885449_biz@gmail.com');
	...
	**/
	function google_return(){
		mail('kulvinder2@gmail.com', 'Google gift  Fail', 'yes');
		...
		if($orderType == 'GIFT'){ // notification for the gift certificates
			...
			if($_status == 'OK' ){ // if payment successfull
				...
				mail('kulvinder2@gmail.com', 'Google checkout Fail', $strReason);
			}
		}
	}
	...
	function sendOrderConfirmationEmail($orderId){
		...
		if(is_array($OrderItems) ){
			...
			foreach($OrderItems as $key=>$item){
				...
				$orderDetailStr .= "<tr><td align=\"left\" colspan=\"3\">Sold by: ".$item['OrderItem']['seller_name']."</td></tr>";
		...
		$this->Email->subject = $template['EmailTemplate']['subject'];
		$this->set('data',$data);
		$this->Email->to = ucfirst($ordData['billing_firstname']).' '.ucfirst($ordData['billing_lastname']).'<'.$ordData['user_email'].'>';
	...
	**/
	function sendOrderDispatchNowEmail($orderId){
		...
		$order_number 	  = $OrderBasicData['Order']['order_number'];
			...
			foreach($OrderSellers as $seller_id){ //
				...
				if(is_array($OrderItems) ){
					...
					foreach($OrderItems as $key=>$item){
						...
						$orderDetailStr .= "<tr><td align=\"left\"><b>Delivery Estimate: </b>".$delDate."</td></tr>";
				...
				$this->Email->to = ucfirst($seller_user_info['User']['firstname']).' '.ucfirst($seller_user_info['User']['lastname']).' <'.$seller_user_info['User']['email'].'>';
	...
	**/
	function giftcertificate_step2(){
		...
		$this->data['Checkout']['paymenthod_method'] = 'Sage';
	...
	function save_giftcertificate($data = null,$total_order = null){
		...
		if(!empty($data)){
			...
			if($this->OrderCertificate->save()){
				...
				$this->data['Certificate']['order_certificate_id'] = $this->OrderCertificate->getLastInsertId();
...
// 		$this->Email->replyTo = Configure::read('replytoEmail');
		...
		if($certificates){
			...
			foreach($certificates as $certificate) {
				...
				*/
				...
				if(!empty($certificate['Certificate']['sender_message'])){
					$data = str_replace('[MessageUserEnteredContent]',$certificate['Certificate']['sender_message'],$data);
				} else{
				...
				$this->Email->to = ucfirst($receiver_name).'<'.$certificate['Certificate']['receiver'].'>';
			...
			$this->Email->to = $user_info_sender['User']['firstname'].' '.$user_info_sender['User']['firstname'].'<'.$user_info_sender['User']['email'].'>';
