...
class SellersController extends AppController
{
	...
	**/
	function beforeFilter(){
		...
		$includeForSeller = array('order_details','orders','refund_order','cancel_order','payment_settings','my_account');
...
// 			}
	...
	*/
	function sign_up_step3(){
		...
		if(!empty($this->data)){
			...
			if($this->data['Seller']['free_delivery']==0){
				
				unset($this->data['Seller']['threshold_order_value']);
			}
			$this->data = $this->cleardata($this->data);
			//$this->data = Sanitize::clean($this->data, array('encode' => false));
	...
	*/
	function admin_index() {
		...
		$totalnoProductSeller = $this->Common->totalnoProductSeller();
		$ProductSellerInStock = $this->Common->inSotckProductSeller();
		$ProductSellerOutSotck = $this->Common->outSotckProductSeller();
		if(!empty($sellers_list)){
			...
			foreach($sellers_list as $seller){
				...
				$order_info =$this->OrderView->find('all',array('conditions' => array(" OrderView.payment_status = 'Y' AND OrderView.deleted != '1' AND OrderView.seller_id =".$seller['User']['id']),'fields'=>array('OrderView.order_id'),'group' =>array('order_id')));
	
				if(!empty($order_info)){
					$sellers_list[$i]['Order']['order_count'] = count($order_info);
				}
				
				if(array_key_exists($seller['User']['id'],$totalnoProductSeller)){
					$sellers_list[$i]['Seller']['totalProducts'] = $totalnoProductSeller[$seller['User']['id']];
				}else{
					$sellers_list[$i]['Seller']['totalProducts'] =  0;
				}
				
				if(array_key_exists($seller['User']['id'],$ProductSellerInStock)){
					$sellers_list[$i]['Seller']['totalProductsInStock'] = $ProductSellerInStock[$seller['User']['id']];
				}else{
					$sellers_list[$i]['Seller']['totalProductsInStock'] =  0;
				}
				
				if(array_key_exists($seller['User']['id'],$ProductSellerOutSotck)){
					$sellers_list[$i]['Seller']['totalProductsOutStock'] = $ProductSellerOutSotck[$seller['User']['id']];
				}else{
					$sellers_list[$i]['Seller']['totalProductsOutStock'] =  0;
				}
				$sellers_list[$i]['Seller']['sellerLastLogin'] =  $this->Common->getUserLastLogin($seller['User']['id']);
				$sellers_list[$i]['Seller']['sellerLastModify'] =  $this->Common->sellerLastModifyProduct($seller['User']['id']);
		...
		if(empty($seller_products)){
			...
			if($this->User->saveField('status', 0 )){
				if(!empty($seller_products)){
					...
					$this->Session->setFlash('This seller is having some products or orders, so we can\'t delete it. This seller and all belonging products have been inactivated successfully.Now, please update minimum price for choiceful.com');
				}
			}else{
		...
		if(!empty($this->data['Seller']['photo']['name'])){
			...
			if( $validationFlag == false){ // if validation fails 
				...
				if(empty($fileName)){
					...
					$this->Session->setFlash('Seller display image has been updated');
				}
			}
			@$this->redirect('/sellers/my_account');
		} else{
	...
	*/
	function admin_upload_bulk_listing($id = null){
		...
		$productConditionsCsv = array(
			'New' => '1',
			'Used-Like New' => '2',
			'Used-Average' => '3',
			'New-Limited Edition' => '4',
			'Used-Limited Edition-Like New' => '5',
			'Used-Limited Edition-Average' => '6',
			'Poor' => '7'
		);
			...
			if($this->data['ProductSeller']['sample_bulk_data']['name'] != '') {
				...
				if( $validationFlag  == true ) {  // if in accepted formats
					...
					if( $columns_count == 16){ // is file is ok to upload
						...
						$countryList = $this->Country->find('list',array('conditions'=>array('Country.status'=>1),'fields'=>array('country_name','id')));
						while (($row = fgetcsv($handle, 4096, ",")) !== FALSE) {
							$first_row_test  = strtoupper(trim($row[0]));
							
							//replace country name with country id modified on 24 May 2013
							if(array_key_exists($row['5'],$countryList)){
								$row['5']= @$countryList[$row['5']];
							}
							//replace condition name with condition id modified on 24 May 2013
							if(array_key_exists($row['7'],$productConditionsCsv)){
								$row['7'] = @$productConditionsCsv[$row['7']];
							}
					...
					//exit;
						...
						if($this->ProductSeller->save($this->data)){
							...
							//echo "<pre>";
							//pr($this->data);
							//print_r($this->ProductSeller->validationErrors);
							//echo "</pre>";
						...
						//$uploadStatus['uploaded_products']	= ($Products_uploaded - $Products_notuploaded);
					...
					if($this->Seller->save($this->data)){
						...
						**/
						...
						if($this->Email->send()) {
							...
							$this->Session->setFlash('The deposit amounts have been saved it may take up to 24 hours for us to verify your payment settings');
						} else{
	...
	*/
	function admin_verify_seller($seller_id = null,$status_value = null,$verify = null){
		...
		if(!empty($this->data) && $verify=='yes'){ 
			...
			$this->data['Seller'] = Sanitize::clean($this->data['Seller']);
			$this->Seller->set($this->data);
			if($this->Seller->save($this->data,array('validate'=>false))){
...
// 		}
	...
	*/
	function summary($seller_id = null,$product_id = 0,$condition_id = null){
		...
		//$sellers_breadcrumb_string = $this->sellersBreadcrumb($seller_id, $product_id,'summary');
		//$this->set('breadcrumb_string', $sellers_breadcrumb_string);
		...
		{
		$sellers_breadcrumb_string = $this->sellersBreadcrumb1($seller_id, 'summary');
		$this->set('breadcrumb_string', $sellers_breadcrumb_string);	
		}
	...
	*/
	function feedback($seller_id = null,$product_id = 0,$condition_id=1){
		...
		// show breadcrumbs add  by pradeep on 19 feb 2013
	        ...
	        if(!empty($product_id))
		{
		$sellers_breadcrumb_string = $this->sellersBreadcrumb($seller_id, $product_id,'feedback');
		$this->set('breadcrumb_string', $sellers_breadcrumb_string);
		}
		else
		
		{
		$sellers_breadcrumb_string = $this->sellersBreadcrumb1($seller_id, 'feedback');
		$this->set('breadcrumb_string', $sellers_breadcrumb_string);	
		}
	...
	*/
	function returns($seller_id = null,$product_id = 0,$condition_id=1){
		...
		if(!empty($product_id))
		{
		$sellers_breadcrumb_string = $this->sellersBreadcrumb($seller_id, $product_id,'returns');
		$this->set('breadcrumb_string', $sellers_breadcrumb_string);
		}
		else
		
		{
		$sellers_breadcrumb_string = $this->sellersBreadcrumb1($seller_id, 'returns');
		$this->set('breadcrumb_string', $sellers_breadcrumb_string);	
		}
	...
	*/
	function get_sellerOrders($filter = 'ALL') {
		...
		if($this->RequestHandler->isAjax()==1){
			...
			$this->set('orders',$orders);
			if ($this->RequestHandler->isMobile()) {
				if(!empty($this->params['named']['testpage'])){
					$this->viewPath = 'elements/mobile/seller' ;
	...
	*/
	...
	function download_all_orders(){
		...
		$fields = array(
			...
			'Order.comments','Order.order_number','Order.created'
		);
		App::import('Model','OrderItem');
		$this->OrderItem = new OrderItem();
		...
		/*$this->Order->bindModel(array("hasOne" =>array('Address' => array(
							'className' => 'Address',
							'fields'=>array('add_phone'),
							'conditions'=> array('Order.user_id = Address.user_id'),
							'foreignKey' => false,
							'type'=>'inner'
							)
						),
					      'belongsTo'=>array('UserSummary' => array(
								'className' => 'User',
								'fields'=>array('id','title','firstname','lastname','email'),
								'foreignKey' => 'user_id',
								'type'=>'inner'
								)
								)
		...
		/*$this->OrderItem->contain(array('Order' => array(
									'UserSummary' ,
									'Address' 
		...
		$csv_output =  "ORDER ID,DATE,ITEM QCID,SELLER REF,PRODUCT NAME,QUANTITY PURCHASED,PRODUCT PRICE,DELIVERY SERVICE,DELIVERY PRICE,GIFT WRAP,GIFT MESSAGE,BUYER PHONE,DELIVERY NAME,DELIVERY ADDRESS 1,DELIVERY ADDRESS 2,DELIVERY TOWN/CITY,DELIVERY COUNTY/STATE,DELIVERY POSTAL CODE/ZIP,DELIVERY COUNTRY,DELIVERY EXPECTED DATE,DELIVERY INSTRUCTIONS,STATUS" ;
		$csv_output .= "\n";
		if(count($orders) > 0){
			App::import('Model','ProductSeller');
			$this->ProductSeller = new ProductSeller();
			...
			foreach($orders as $value){
				...
				$value['Order']['shipping_firstname'].' '.$value['Order']['shipping_lastname']).",".str_replace(",",' || ',
				...
				$value['Order']['comments']).",".str_replace(",",' || ',
		...
		$filePath="all_seller_orders_listing_".$orderNo.'_'.date("Ymd").".csv";
		header("Content-type: application/vnd.ms-excel");
		header("Content-Disposition: attachment; filename=".$filePath."");
		header("Pragma: no-cache");
		header("Expires: 0");
...
*/
	...
	function download_unshipped_orders(){
		...
		if(count($new_order_array) > 0){
			...
			foreach($new_order_array as $new_value){
				...
				$buyer_ph['Address']['add_phone']).",".str_replace(",",' || ',
				$new_value['Order']['shipping_firstname'].' '.$new_value['Order']['shipping_lastname']).",".str_replace(",",' || ',
				...
				$country).",".str_replace(",",' || ',
				$expected_delivery_date).",".str_replace(",",' || ',
				$new_value['Order']['comments']).",".str_replace(",",' || ',
				$shippingStatus['OrderSeller']['shipping_status']).",\n";
		...
		$orderNo=$new_value['Order']['order_number'];		
		//$filePath="unshipped_sellers_orders_listing_".date("Ymd").".csv";
		$filePath="unshipped_sellers_orders_listing_".$orderNo.'_'.date("Ymd").".csv";
		header("Content-type: application/vnd.ms-excel");
		header("Content-Disposition: attachment; filename=".$filePath."");
		header("Pragma: no-cache");
		header("Expires: 0");
	...
	*/
	function order_details($order_id = null,$editorder = null){
		...
		//REF #I1474
		$refund_details = $this->OrderRefund->find('all',array('conditions'=>array('OrderRefund.order_id'=>$order_id,'OrderRefund.seller_id'=>$seller_id)));
		$total_refundes = 0;
		if(!empty($refund_details)){
			$total_refundes = $this->OrderRefund->find('all',array('conditions'=>array('OrderRefund.order_id'=>$order_id,'OrderRefund.seller_id'=>$seller_id),'fields'=>array('sum(amount) as total_amount')));
				...
				if(!empty($qty_flag)){
					if(!empty($this->data['OrderSeller']['Items'])){
						if(!empty($this->data['OrderSeller']['shipment_number'])){
							...
							foreach($this->data['OrderSeller']['Items'] as $item_id => $item_detail){
								if(!empty($item_detail['dispatch_qty'])){
									...
									$DispatchedItemQty = "";
									$ship_number_infos = $this->DispatchedItem->find('all',array('conditions'=>array('DispatchedItem.seller_id'=>$seller_id,'DispatchedItem.order_id'=>$order_id,'DispatchedItem.order_item_id'=>$item_id),'order'=>array('DispatchedItem.created Desc'),'fields'=>array('DispatchedItem.shipment_number','sum(DispatchedItem.quantity) as dispatchQuantity')));
									$DispatchedItemQty = $ship_number_infos['0']['0']['dispatchQuantity'];
										
									if(empty($DispatchedItemQty)){
										...
										$RestQuantity = ($DispatchedItemQty+$item_detail['dispatch_qty']);
										
										if(($item_detail['total_ordered_qty']+1) > $RestQuantity){
										...
										if(empty($ship_product_str)){
											...
											$this->Session->setFlash('Order has been updated.','default',array('class'=>'flashError'));
										}
									$this->Session->setFlash('Order has been shiped.','default',array('class'=>'flashError'));
									}
								}
							}
						...
						$ship_number_info = $this->DispatchedItem->find('first',array('conditions'=>array('DispatchedItem.seller_id'=>$seller_id,'DispatchedItem.order_id'=>$order_id),'order'=>array('DispatchedItem.created Desc'),'fields'=>array('DispatchedItem.shipment_number')));
							...
							$this->data = '';
							$this->sendShipmentMail($mail_data);
							$this->Session->write('successUpdate', 1);
							$this->Session->setFlash('Your selected items have been shipped. You will be redirected within <span id="counter"></span> seconds');
							//$this->Session->setFlash('Your selected items have been shipped');
							$this->redirect('/sellers/ship_order/'.base64_encode($order_id).'/'.base64_encode('edit'));
						}
					}
				} else{
...
// 					}
				...
				if(!empty($qty_flag)){
					if(!empty($this->data['OrderSeller']['Items'])){
						if(!empty($this->data['OrderSeller']['shipment_number'])){
							...
							$this->Session->write('successUpdate', 1);
							$this->Session->setFlash('Shipment updated. You will be redirected within <span id="counter"></span> seconds');
							$this->redirect('/sellers/ship_order/'.base64_encode($order_id).'/'.base64_encode('edit'));
						} else{
							$ship_number_info = $this->DispatchedItem->find('first',array('conditions'=>array('DispatchedItem.seller_id'=>$seller_id,'DispatchedItem.order_id'=>$order_id),'order'=>array('DispatchedItem.created Desc'),'fields'=>array('DispatchedItem.shipment_number')));
							$shipment_number ='';
							...
							$this->sendShipmentMail($mail_data);
							$this->Session->setFlash('You can not ship this order as it was cancelled or modified by the customer recently - please check your email notifications we sent this to your registered customer service email address');
							//$this->redirect('/sellers/ship_order/'.base64_encode($order_id).'/'.base64_encode('edit'));
							$this->redirect('/sellers/orders/');
						}
					}
	...
	*/
	function get_order_details($order_id = null){
		...
		$this->OrderSeller->expects(array('SellerSummary','Order','Seller'));
		$this->OrderSeller->Order->expects(array('UserSummary'));
		$this->OrderSeller->recursive = 2;
		$seller_id = $this->Session->read('User.id');
...
//End Comment lines are on 1-Feb-2013
		...
		if(!empty($this->data)){
			...
			//Start Comment lines are on 1-Feb-2013
				...
				$this->data['OrderRefund']['reason_id'] = $this->data['OrderSeller']['reason_id'];*/
			...
			//End Comment lines are on 1-Feb-2013
				...
				//Start Comment lines are on 1-Feb-2013
					$this->Ordercom->cancelOrderAllitems($order_id,$seller_id,$this->data['OrderSeller']['reason_id'], $order_info_for_refund['Order']['user_id']);
					
					//$this->Session->setFlash('The selected item(s) have been cancelled');
					$this->Session->write('successUpdate', 1);
					$this->Session->setFlash('The selected item(s) have been cancelled. You will be redirected within <span id="counter"></span> seconds.');
	...
	function refund_order($order_id = null){
		...
		if(!empty($this->data)){
			...
			if($this->OrderSeller->validates()) { 
				...
				$refundAmount = number_format($this->data['OrderRefund']['amount'],2,'.','');
				$refundUpAmount = number_format($this->data['OrderRefund']['up_to_refund'],2,'.','');
					
				$this->OrderRefund->set($this->data);
				if($this->OrderRefund->save()) {
					$this->sendRefundMailToCustomer();
					//REF F1489
					//if($refundUpAmount == $refundAmount){
						//$this->Order->id = $order_id;
						//Because there are multiple seller in a order and we can't get all seller are cancle order through all seller
						//$this->Order->saveField('deleted','3');
						//REF F1489
						//$this->OrderSeller->query(" update order_sellers SET shipping_status = 'Cancelled'  where order_id = $order_id and seller_id = $seller_id ") ;
					//}
					//$message = "You have successfully authorized a refund to the customer. <a href = ".SITE_URL."pages/view/returns-and-refunds>Learn more</a>";
					
					$this->Session->write('successUpdate', 1);
					$message = "You have successfully authorized a refund to the customer. You will be redirected within <span id='counter'></span> seconds. <a href = ".SITE_URL."pages/view/returns-and-refunds>Learn more</a>";
	...
	*/
	function sendRefundMailToCustomer(){
		...
		*/
		...
		$or_number = '<a href="'.SITE_URL.'orders/order_history?utm_source=Refund+Initiated+for+Order&amp;utm_medium=email">'.$this->data['OrderRefund']['order_number'].'</a>';
		$data = str_replace('[OrderNumber]',$or_number,$data);
		$data = str_replace('[RefundAmount]',CURRENCY_SYMBOL.number_format($this->data['OrderRefund']['amount'],2),$data);
		...
		if(!empty($this->data)){
			...
			if($this->OrderSeller->validates()) {
				...
				if(!empty($qty_flag)){
					...
					$this->Session->write('successUpdate', 1);
					$this->Session->setFlash('Order Items cancelled successfully. You will be redirected within <span id="counter"></span> seconds.');
					
					$this->redirect('/sellers/cancel_orderitems/'.base64_encode($order_id));
				} else{
					$this->Session->setFlash('Select the product(s) you would like to cancel','default',array('class'=>'flashError'));
				}
			} else{
				$errorArray = $this->OrderSeller->validationErrors;
				$this->set('errors',$errorArray);
	...
	function sendShipmentMail($mailData = null){
		...
		if(!empty($mailData['order_id'])){
			...
			if(!empty($order_items)){
				if(!empty($order_items[0]['OrderItem']['estimated_delivery_date'])){
					...
					if(!empty($sellersArr)){
						foreach($sellersArr as $seller_id=>$seller_name){
							if(empty($seller_name_str)){
								$seller_name_str_all = '<a href="'.SITE_URL.'sellers/summary/'.$seller_id.'?utm_source=Order+Shipped&amp;utm_medium=email">'.$seller_name.'</a>';
							} else{
								$seller_name_str_all = $seller_name_str.' , '.'<a href="'.SITE_URL.'sellers/summary/'.$seller_id.'?utm_source=Order+Shipped&amp;utm_medium=email">'.$seller_name.'</a>';
							}
						}
					}
					//Add send only seller display name 
					$sellerInfo = $this->Common->getSellerInfo($mailData['seller_id']);
					$seller_name_str = '<a href="'.SITE_URL.'sellers/summary/'.$mailData['seller_id'].'?utm_source=Order+Shipped&amp;utm_medium=email">'.$sellerInfo['Seller']['business_display_name'].'</a>';
					//End send only seller display name
					...
					if(!empty($sellersArr)){
						foreach($sellersArr as $seller_id=>$seller_name){
							if(empty($seller_name_str)){
								$seller_name_str = '<a href="'.SITE_URL.'sellers/summary/'.$seller_id.'?utm_source=Order+Shipped&amp;utm_medium=email">'.$seller_name.'</a>';
							} else{
								$seller_name_str = $seller_name_str.' , '.'<a href="'.SITE_URL.'sellers/summary/'.$seller_id.'?utm_source=Order+Shipped&amp;utm_medium=email">'.$seller_name.'</a>';
							}
						}
					}
			...
			*/
			...
			$data = str_replace('[OrderNumberLink]','<a href="'.SITE_URL.'orders/order_history/?utm_source=Order+Shipped&amp;utm_medium=email">'.$order_info['Order']['order_number'].'</a>',$data);
			...
			*/
			...
			$data = str_replace('[SellersDisplayNameAll]',$seller_name_str_all,$data);
		...
		if(!empty($this->data['Search'])){
			...
			if($value !="") {
				if(trim($fieldname)=='All'){
					...
					if(empty($criteria)){
						$criteria = "User.firstname LIKE '%".$first_name."%' OR User.lastname LIKE '%".$last_name."%' OR SellerInfo.business_display_name LIKE '%".$value1."%' OR OrderItem.product_name LIKE '%".$value1."%' OR User.email LIKE '%".$value1."%' OR Product.quick_code LIKE '%".$value1."%'";
					} else {
						$criteria .= " and (User.firstname LIKE '%".$first_name."%' OR User.lastname LIKE '%".$last_name."%' OR SellerInfo.business_display_name LIKE '%".$value1."%' OR OrderItem.product_name LIKE '%".$value1."%' OR User.email LIKE '%".$value1."%' OR Product.quick_code LIKE '%".$value1."%')";
					}
				} else {
					if(trim($fieldname)!=''){
						if($fieldname == 'customer_name'){
							...
							if(empty($criteria)){
								$criteria = "SellerInfo.business_display_name LIKE '%".$value1."%'";
							} else {
								$criteria .= " and (SellerInfo.business_display_name LIKE '%".$value1."%')";
							}
							...
							if(empty($criteria)){
								$criteria = "User.email LIKE '%".$value1."%'";
							} else {
								$criteria .= " and (User.email LIKE '%".$value1."%')";
							}
						}
						else if($fieldname == 'quick_code'){
							if(empty($criteria)){
								$criteria = "Product.quick_code LIKE '%".$value1."%'";
							} else {
								$criteria .= " and (Product.quick_code LIKE '%".$value1."%')";
							}
						}else {
						}
					}
...
// 				'SellerInfo.user_id',
		...
		$this->Feedback->expects(array('User','SellerSummary','OrderItem','SellerInfo','Product'));
...
// pr($this->data); die;
	...
	*/
	...
	/********************************************** */
	/** 
	@function:	sellersBreadcrumb	
	@description	create breadcrumb for seleers page details page for front end side	
	@Created by: 	Pradeep kumar
	@params		seller id & product id
	@Modify: sellersBreadcrumb
	@Created Date:	20 Feb 2013	
	*/
	
	function sellersBreadcrumb($seller_id=null, $product_id=null, $sellertype=null){
		...
		$link_seperator = " > " ;
		$strLink = '<li><a class="" href="/">Choiceful</a></li>';
		$strLink .= '<li><a href="'.SITE_URL.''.$dept_url_name.'/departments/index/'.$department_id.'" >' .$department_name. '</a></li>';
		...
		//$strLink .= $link_seperator;
			...
			if($sellertype =='summary')
			{
			$sllerstr = 'Choiceful Marketplace Seller Summary for'; 
			}
			else if($sellertype =='feedback')
			{
			$sllerstr = 'Choiceful Marketplace All Feedback for'; 
			}
			else if($sellertype =='returns')
			{
			$sllerstr = ' Choiceful Marketplace Returns Policy for'; 
			}
			
			$seller_info = $this->Seller->find('first',array('conditions'=>array('Seller.user_id'=>$seller_id),'fields'=>array('Seller.business_display_name')));
			
			$finalArr[$seller_id] = $sllerstr.' '.$seller_info['Seller']['business_display_name'];
			...
			if( is_array($finalArr) ){
				...
				foreach($finalArr as $key=>$value){
					...
					else if($j == ($totalCount-2)){
					
					$cat_url_name = str_replace(array('&',' '), array('and','-'), html_entity_decode($value, ENT_NOQUOTES, 'UTF-8'					));	
					$innerlink = SITE_URL.$dept_url_name.'/'.$cat_url_name.'/categories/viewproducts/'.$key.'/'.$department_id;
				...
				$product_url = $this->Common->getProductUrl($product_id);			
				$innerlink = SITE_URL.$product_url.'/categories/productdetail/'.$product_id;
				...
				//$strLink .= $link_seperator;
						...
						$innerlink = SITE_URL.$dept_url_name.'/'.$cat_url_name.'/categories/index/'.$key;
						$strLink .='<li><a href="'.$innerlink.'">' .$value. '</a></li>';
						//$strLink .= $link_seperator;
					}
				}
	...
	function sellersBreadcrumb1($seller_id=null, $sellertype=null){
		...
		$strLink .= '<a href="'.SITE_URL.'/marketplaces/sales_report">Sales Report</a>';
		...
		$strLink .= $link_seperator;
			...
			$strLink .= $sllerstr.' '.$seller_info['Seller']['business_display_name'];
